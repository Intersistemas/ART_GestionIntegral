...
export class ArtAPIClass extends ExternalAPI {
  ...
  private readonly rgrlBasePath = process.env.NEXT_PUBLIC_API_RGRL_URL || 'http://arttest.intersistemas.ar:8302';

  private getRgrlURL = ({ path = "", search }: ExternalAPIGetURLParams = {}) => {
    const url = new URL(path, this.rgrlBasePath);
  ...
  //#endregion
    ...
    getFormulariosRGRL = async (cuit: number, all: boolean = false): Promise<ApiFormularioRGRL[]> => {
      const url = this.getRgrlURL({ path: "/api/FormulariosRGRL", search: { CUIT: String(cuit) } });
      if (all) url.searchParams.set("pageSize", "99999");
    ...
    getEstablecimientosEmpresa = async (cuit: number): Promise<ApiEstablecimientoEmpresa[]> => {
      const url = this.getRgrlURL({ path: `/api/Establecimientos/Empresa/${encodeURIComponent(cuit)}` });
      const res = await fetch(url.toString(), { cache: "no-store", headers: { Accept: "application/json" } });
      if (res.status === 404) return [];
      if (!res.ok) {
        const raw = await res.text().catch(() => "");
