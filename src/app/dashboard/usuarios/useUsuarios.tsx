...
export const useUsuarios = () => {
  const { data: usuariosData, error: usuariosError, isLoading: usuariosLoading, mutate: mutateUsuarios } = useApiDataFetching<{ data: UsuarioRow[] }, "UsuarioAPI", "getAll">(
    "UsuarioAPI", "getAll", { params: { EmpresaId: 1 } }
);

  const { data: roles, error: rolesError, isLoading: rolesLoading } = useApiDataFetching<RolesInterface[], "UsuarioAPI", "getRoles">(
  "UsuarioAPI", "getRoles"
);
  ...
  const registrarUsuario = async (formData: any) => {
    try {
      ...
      await fetcher<any, "UsuarioAPI", "registrar">("UsuarioAPI", "registrar", { data: formData });
